...
def test_generate_uniform_random_stable():
    file_name = os.path.join(os.path.dirname(__file__), 'pos_test.5')
    _check_remove_file(file_name)
    ...
    box = [12.25111212,
           7.129525025,
           11.09029595]
    generate_uniform_random(n_particles, box, stable=True, path=file_name,
                            threshold=0.1, max_iter=100)
    assert os.path.exists(file_name)

    x_all = np.zeros(n_particles)
    y_all = np.zeros(n_particles)
    z_all = np.zeros(n_particles)
    ...
    with open(file_name, 'r') as in_file:
        ...
        for i in range(1, n_particles+1):
            ...
            assert len(line) == 13
            x, y, z = [float(p) for p in line[4:7]]
            x_all[i-1], y_all[i-1], z_all[i-1] = x, y, z
            assert x <= box[0]
            assert y <= box[1]
            assert z <= box[2]
